...
//	private Logger logger = LoggerFactory.getLogger(YahooWeatherService.class);
	...
	public String getWoeid(String text) throws Exception{
		//String urlParam = "http://yahooapis.com/geo/placefinder?text=mumbai&flags=J&oauth_consumer_key=dj0yJmk9Nlo5RldRanR4S3U4JmQ9WVdrOVFVMXlXR0YzTlRBbWNHbzlNQS0tJnM9Y29uc3VtZXJzZWNyZXQmeD1hZQ--";
...
return woeid;
	...
	public String getCondition(String condition){
		...
		final Set<String> cloudy = new HashSet<String>(Arrays.asList("Cloudy","Mostly Cloudy","Mostly Cloudy (night)","Mostly Cloudy (day)","Partly Cloudy (night)","Partly Cloudy (day)","Tornado","Tropical Storm","Hurricane","Severe Thunderstorms","Thunderstorms","Mixed Rain and Snow","Mixed Rain and Sleet","mixed snow and sleet","Freezing Drizzle","Drizzle","Freezing Rain","Showers","Showers","Isolated Thunderstorms","Scattered Thunderstorms","Scattered Thunderstorms","Scattered Showers","Partly Cloudy"));
		final Set<String> sunny = new HashSet<String>(Arrays.asList("Windy","Clear (night)","Sunny","Fair (night)","Fair (day)","Hot","Fair","Not Available","Mostly Clear","Breezy"));
		final Set<String> snow = new HashSet<String>(Arrays.asList("Thundershowers","Snow Showers","Isolated Thundershowers","Mixed Rain and Hail","Snow Flurries","Light Snow Showers","Blowing Snow","Snow","Hail","Sleet","Dust","Foggy","Haze","Smoky","Blustery","Cold","Heavy Snow","Scattered Snow Showers","Heavy Snow"));
		String cond = (cloudy.contains(condition)) ? "Rain" : ((sunny.contains(condition) ? "Sunny" : (snow.contains(condition) ? "Snow" : null))) ;
